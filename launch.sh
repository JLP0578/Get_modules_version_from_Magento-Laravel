...
function recup_module_magento {
    ...
    docker run --rm --name $MAGENTO_NAME --network $NETWORK -v $MAGENTO_LOCALCOMPOSERJSON:$CONTAINERCOMPOSERJSON:$READONLY -v $MAGENTO_LOCALCOMPOSERLOCK:$CONTAINERCOMPOSERLOCK:$READONLY -v $MAGENTO_LOCALAUTHJSON:$CONTAINERAUTHJSON:$READONLY -v $LOCALPROCESSSH:$CONTAINERPROCESSSH -v $LOCALENV:$CONTAINERENV -it $IMAGE sh -c "$argument"
}
