...
function recup_module_laravel {
    ...
    docker run --rm --name $LARAVEL_NAME -e CONTAINER_NAME=$LARAVEL_NAME --network $NETWORK -v $LARAVEL_LOCALCOMPOSERJSON:$CONTAINERCOMPOSERJSON:$READONLY -v $LARAVEL_LOCALCOMPOSERLOCK:$CONTAINERCOMPOSERLOCK:$READONLY -v $LOCALPROCESSSH:$CONTAINERPROCESSSH -v $LOCALENV:$CONTAINERENV -it $IMAGE sh -c "$argument"
}
